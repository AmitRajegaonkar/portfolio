...
export function ProjectCard({
  ...
  // Show a GitHub icon for repo links, a globe for live-site links.
  const link = project.github?.trim();
  const isRepo = !!link && /github\.com/i.test(link);
  const LinkIcon = isRepo ? Github : Globe;
  const linkLabel = isRepo
    ? `${project.title} on GitHub`
    : `${project.title} — live site`;

  return (
    // `relative` + a stretched overlay link keeps the whole card clickable
    // while letting the repo/live icon sit on top as its own separate link.
    <Card className="group relative flex h-full flex-col overflow-hidden transition-colors duration-300 hover:border-accent/60">
      ...
      {/* Repo / live-site icon — sits above the stretched link (z-20) */}
      {link ? (
        <a
          href={link}
          target="_blank"
          ...
          title={isRepo ? "View source" : "Visit live site"}
